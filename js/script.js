...
function updateBackgroundColor(){
  const colors = ['red', 'blue', 'green', 'pink', 'yellow', 'black', 'purple'];
...
function printQuote(){
  ...
  let output = `<p class="quote">${randomQuote.quote}</p>
    ...
    if(randomQuote.tags){
      output+= `<br><span class="tags"> tags: ${randomQuote.tags}</span>`;
    }
    output+= `</p><img src= "${images[randomQuote.source]}">`;
...
//Set interval to change the quote every 10 secs
...
***/
//allow user to change the quote on click
